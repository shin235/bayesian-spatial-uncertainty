...
def build_bayesian_pm25(input_dim_phi, input_dim_cov, kl_weight=None):
    ...
    phi_dense = tf.keras.layers.Dropout(
        rate=DROPOUT_RATE)(phi_dense, training=True)

    phi_dense = tfpl.DenseVariational(
        100, prior_dist_fn, posterior_dist_fn, kl_weight=kl_weight, activation='relu')(phi_dense)
    ...
    cov_dense = tf.keras.layers.Dropout(
        rate=DROPOUT_RATE)(cov_dense, training=True)

    # Concatenate
    merged = tf.keras.layers.Concatenate()([phi_dense, cov_dense])
    merged = tf.keras.layers.Dense(128, activation='relu')(merged)
    merged = tf.keras.layers.Dropout(rate=DROPOUT_RATE)(merged, training=True)
    merged = tf.keras.layers.Dense(128, activation='relu')(merged)
    merged = tf.keras.layers.Dropout(rate=DROPOUT_RATE)(merged, training=True)

    merged = tfpl.DenseVariational(
        100, prior_dist_fn, posterior_dist_fn, kl_weight=kl_weight, activation='relu')(merged)
